/* Morhological Operation- Shrinking
   ...
   This code also counts number of objects after they have been shrunk, and their sizes. My code is for a stars image, where stars haven been shrunk and counted
...

{
    ...
    for (int i = 0; i < ySize; i++)
    {
        for (int j = 0; j < xSize; j++)
        {
            ...
            result[i*xSize+j] = (resultint[i][j]*255);
        }
    }
    for (int i = 0; i < ySize; i++)
    {
    ...
    for (int i = 0; i < 13; i++)
    {
        starcount += countsize[i];
        if (countsize[i] != 0)
            count1++;

        cout << " stars of size " << i+1 << " are " << countsize[i] << endl;
    }
    cout << "no of sizes are " << count1 << endl;
    cout << "no of stars are " << starcount << endl;
